...
class Competitive(nn.Module):
    ...
    def forward(self, y, t=None, lrn=False):
        ...
        if self.random_abstention_on:
            ...
            if self.random_abstention == self.HARD_RAND_ABST: scores = y * (
                        torch.rand_like(abst_prob) >= abst_prob).float().unsqueeze(0)
        ...
        if lrn and self.lfb_on and self.gamma is not None and self.training: self.lfb_kernel = self.lfb_kernel.pow(
            self.gamma)
...
class HebbianConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        if self.weight_zeta_mode is not None:
            self.weight_ada = nn.Parameter(torch.empty(out_channels, in_channels, kernel_size[0], kernel_size[1]),
                                           requires_grad=True)
            self.weight_ada = weight_init(self.weight_ada)
            self.weight_zeta = weight_zeta
            if self.weight_zeta_mode == self.ZETA_MODE_PARAM: self.weight_zeta = nn.Parameter(
                torch.tensor(weight_zeta).float(), requires_grad=True)
            if self.weight_zeta_mode == self.ZETA_MODE_VEC: self.weight_zeta = nn.Parameter(
                weight_zeta * torch.ones(out_channels).float().view(-1, 1, 1, 1), requires_grad=True)
            if self.weight_zeta_mode == self.ZETA_MODE_MAT: self.weight_zeta = nn.Parameter(
                weight_zeta * torch.ones_like(self.weight_ada).float(), requires_grad=True)
        ...
        if self.bias_zeta_mode is not None:
            ...
            if self.bias_zeta_mode == self.ZETA_MODE_PARAM: self.bias_zeta = nn.Parameter(
                torch.tensor(bias_zeta).float(), requires_grad=True)
            if self.bias_zeta_mode in [self.ZETA_MODE_VEC, self.ZETA_MODE_MAT]: self.bias_zeta = nn.Parameter(
                bias_zeta * torch.ones(out_channels).float(), requires_grad=True)
    ...
    def apply_act(self, s, lrn=False, cpl=True):
        ...
        if self.conserve_var and self.var_adaptive: y = y * (
                    self.trk.running_var.view(1, -1, 1, 1) + self.trk.eps) ** 0.5
    ...
    def compute_update(self, x):
        ...
        if self.using_adaptive_weight or self.using_adaptive_bias or self.var_adaptive or self.act_complement_adapt is not None:
            ...
            s = self.lrn_sim(x, self.get_weight(),
                             self.get_bias() if self.lrn_bias else None)  # Compute similarity metric between inputs and weights
            ...
            if self.using_adaptive_weight or self.using_adaptive_bias:
                ...
                if self.using_adaptive_weight:
                    ...
                    for grp in range(
                            2):  # repeat the computation for the two neuron groups using complementary nonlinearities
                        if grp == 1 and not self.act_complement_grp: break
                        grp_slice = slice(self.weight.size(0))
                        if self.act_complement_grp: grp_slice = slice(
                            self.act_complement_from_idx) if grp == 0 else slice(self.act_complement_from_idx,
                                                                                 self.weight.size(0))
